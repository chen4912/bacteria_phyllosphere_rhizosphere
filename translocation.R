...
###permanova analysis
rm(list=ls())
total <- readRDS("trans_wunifrac.rds")
...
total <- readRDS("trans_shared_wunifrac.rds")
total <- data.frame(as.matrix(total))
group <- read.xlsx("field_translocation.xlsx", sheet = "group",colNames = T, rowNames = T)
group$Site <- as.factor(group$Site)
group$Popu <- as.factor(group$Popu)
...
total <- readRDS("trans_unique_wunifrac.rds")
total <- data.frame(as.matrix(total))
group <- read.xlsx("field_translocation.xlsx", sheet = "group",colNames = T, rowNames = T)
group$Site <- as.factor(group$Site)
group$Popu <- as.factor(group$Popu)
...
p1 <- create_plot(df1, "ASVs.of.total", "No. of total ASVs (log)", 
                  c("#6D9DFF", "#70D3ED", "#B79F00"), c("#6D9DFF", "#70D3ED", "#B79F00"), c(2.4, 3.0))
...
p2 <- create_plot(df2, "ASVs.of.total", "No. of total ASVs (log)", 
                  c("#E09513", "#9962EF"), c("#E09513", "#9962EF"), c(2.2, 3.0))
p2
...
p3 <- create_plot(df1_shared, "ASVs", "% ASVs shared", 
                  c("#6D9DFF", "#70D3ED", "#B79F00"), c("#6D9DFF", "#70D3ED", "#B79F00"), c(4, 9))
p3
...
p4 <- create_plot(df2_shared, "ASVs", "% ASVs shared", 
                  c("#E09513", "#9962EF"), c("#E09513", "#9962EF"), c(2, 10))
p4
...
####Multiple comparisons (based on non-mixed models and mixed models)##
group <- read.xlsx("field_translocation.xlsx", sheet = "group", colNames = TRUE, rowNames = TRUE)
group$OTUs.of.total <- log10(group$OTUs.of.total)
##ASVs.of.total    |    ASVs     |    read.of.shared
group$group <- paste0(group$Compartment,"_",group$Site)
mid <- subset(group, Type == "MM" | Type == "ML")
model <- aov(read.of.shared ~ group, mid)
...
mid <- subset(group, Type == "HH" | Type == "HM" | Type == "HL")
model <- aov(read.of.shared ~ group, mid)
model_means <- emmeans(object = model,
                       specs = "group")
model_means_cld <- cld(object = model_means,
...
group$Site <- as.factor(group$Site)
group$Popu <- as.factor(group$Popu)
group <- subset(group,Popu != "L")
###ASVs.of.total
model <- lmer(ASVs.of.total ~ Compartment * Site * Popu + (1|pair), 
              na.action= na.omit,data = group)
car::Anova(model, type = 3)
ranova(model)

emm1 = emmeans(model, specs = pairwise ~ Compartment *Site|Popu, type = 'response', adjust = 'tukey')
...
emm1 = emmeans(model, specs = pairwise ~ Compartment *Site|Species* Popu, type = 'response', adjust = 'tukey')
emm1_multi = multcomp::cld(emm1,alpha=0.05,Letters=letters,adjust="none",decreasing = T)
emm1_multi$.group <- trimws(emm1_multi$.group)
emm1_multi
...
#############Fig4#############
###entire
ASV <- readRDS("trans_wunifrac.rds")
group <- read.xlsx("field_translocation.xlsx", sheet = "group", colNames = TRUE, rowNames = TRUE)
ASV1 <- data.frame(as.matrix(ASV))

# Define a function to handle PCoA and plot
...
p4a_1 <- perform_pcoa_plot(group1, "Phyllosphere")

# rhizosphere
group2 <- subset(group, Compartment == "R" & (Type %in% c("HH", "HM", "HL", "MM", "ML")))
p4a_2 <- perform_pcoa_plot(group2, "Rhizosphere")
...
####Fig4c
group <- read.xlsx("field_translocation.xlsx", sheet = "group",colNames = T, rowNames = T)
bacteria_unifrac1 <- readRDS("trans_wunifrac.rds")
...
#group <- read.xlsx("new_data_trans.xlsx", sheet = "pair_dissimilarity",colNames = T, rowNames = T)
#results_entire <- subset(results_entire, elevation != "0")
...
p4c <- ggplot() +
  geom_point(results_entire, mapping = aes(x = elevation, y = Value, color = Species),size = 1.5,shape = 21,alpha = 0.5) +
...
###Fig4d
group <- read.xlsx("field_translocation.xlsx", sheet = "Proportion",colNames = T, rowNames = T)
###gamm拟合
fit1<-gamm(formula(Proportion~s(distance,bs='cr', k = 3)), 
           random=list(Species=~1,Site = ~1),data=group,
           family=gaussian,method = 'REML')
draw(fit1$gam)+theme_bw()
summary(fit1$gam)

p4d <- ggplot() +
  geom_point(group, mapping = aes(x = distance, y = Proportion, color = Species),size = 3,shape = 21,alpha = 0.5) +
  stat_smooth(data = group,
              aes(x = distance, y = Proportion),
              method = "gam", formula = y ~ s(x, k = 3),se = TRUE, linewidth = 1, color = "black") +
  theme_bw()+
  scale_color_manual(values = c("#6D6D67","#F6B16B","#B84A23")) +
  labs(x = "Transplantation elevational distance (m)",y = "Proportion of taxa \nin inital transplanted soils (%)", title = NULL) +
  theme_classic()+
  theme(axis.title = element_text(color='black',size=12),
        axis.title.x=element_text(colour='black', size=12,vjust = 0),
        axis.title.y=element_text(colour='black', size=12,vjust = 0),
        ...
        legend.key=element_blank(),
        legend.position = "right",
        legend.background = element_rect(colour = "black"));p4d
...
df <- data_summary(group, varname = "lnHome_CK", groupnames = c("Elevational", "Species"))
df$Elevational <- factor(df$Elevational,levels = c("356","695","1051"))
df$CI <- 1.96*df$se

ggplot(data = df) +
  geom_errorbar(mapping = aes(x = Elevational, y = lnHome_CK, ymin = lnHome_CK - CI, ymax = lnHome_CK + CI, 
...
ggplot() +
  geom_jitter(data = group1, 
              aes(x = Elevational, y = ratio_Home_CK, color = Species), width = 0.2,
              height = 0, size = 2.5, shape = 21, 
              stroke = 0.3, alpha = 0.7,
              show.legend = FALSE) +
  stat_smooth(data = group1,
              aes(x = Elevational, y = ratio_Home_CK),
              method = "gam", formula = y ~ s(x, k = 3),se = TRUE, linewidth = 1, color = "black") +
  theme_bw() +
  scale_color_manual(values = c("#6D6D67","#F6B16B","#B84A23")) +
  labs(x = "Transplantation\n elevational distance (m)",y = "PGR in transplanted site") +
  scale_y_continuous(labels = scales::label_comma(accuracy = 0.01)) +
  theme(panel.grid.major = element_blank(),
        legend.position = "none",
...
#fit<-gamm(formula(lnHome_CK~s(distance,bs='cr', k = 6)), 
#         random=list(Species=~1),data=group1,
#        family=gaussian,method = 'REML')
#draw(fit$gam)+theme_bw()
#summary(fit$gam)
...
group <- subset(group,Popu != "L")
###ASVs.of.total
model <- lmer(ASVs.of.total ~ Compartment *Site * Species* Popu + (1|pair), 
              na.action= na.omit,data = group)
car::Anova(model, type = 3)
ranova(model)
...
#####################FigS8##################
OTU <- readRDS("trans_unique_wunifrac.rds")
group <- read.xlsx("field_translocation.xlsx", sheet = "group", colNames = TRUE, rowNames = TRUE)
